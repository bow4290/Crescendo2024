...
public class NewWrivot extends SubsystemBase {
  ...
  public static final double TOLERANCE_WRIST = 0.02;
  ...
  public NewWrivot(){
    ...
    configurationWrist.Slot0.kP = 0.15;
  ...
  public Command cmdGoToState(BotStates targetState){
     SequentialCommandGroup cmdGroup = new SequentialCommandGroup(
      ...
      cmdGoToDegree(motorWrist, BotStates.STASH.getWristDeg(), GEAR_RATIO_WRIST).until(() -> isWristFinished(BotStates.STASH)),
      cmdGoToDegree(motorPivot1, targetState.getPivotDeg(), GEAR_RATIO_PIVOT).until(() -> isPivotFinished(targetState)),
      cmdGoToDegree(motorWrist, targetState.getWristDeg(), GEAR_RATIO_WRIST).until(() -> isWristFinished(targetState)),
  ...
  private Command cmdGoToDegree(TalonFX targetMotor, double targetDegrees, double gearRatio){
    double positionRotations = Conversions.degToRotationsGearRatio(targetDegrees, gearRatio);

    return this.startEnd(() -> {
      targetMotor.setControl(positionRequest.withPosition(positionRotations));
    }, 
    () -> {
